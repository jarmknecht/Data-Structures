...
namespace cs235
{
	...
	public:
		...
		ArrayList(size_t initialSize = 16)
		{
			count = 0;
		...
		{
			if (pos >= GetCount()) {
				throw std::exception("Hi Professor. Cannot get value outside of the current array size.");
			}
			return data[pos];
		}
		...
		void InsertAt(size_t pos, const T& value)
		{
			...
			if (pos > GetCount()) {
				throw std::exception("Hi Professor. Cannot insert outside of the array count.");
			}
		...
		{
			if (pos >= GetCount()) {
				throw std::exception("Hi Professor. Cannot remove outside of the array count.");
			}
		...
		{
			if (pos >= GetCount()) {
				throw std::exception("Hi Professor. Cannot set value outside of the array count.");
			}
			data[pos] = value;
		}
	};
